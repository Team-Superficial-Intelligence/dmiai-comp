...
def split_img(img: np.ndarray, fig_size=(110, 110)) -> List[List[np.ndarray]]:
    ...
    result_list = [
        [np.zeros((fig_size[0], fig_size[1], 3)) for _ in range(w)] for _ in range(h)
    ]
    i = 0
    for h_pos, w_pos in positions:
        ...
        y = h_pos * fig_size[1]
        new_img = img[
            y : y + fig_size[1], x : x + fig_size[0],
        ]
        try:
            assert new_img.shape == result_list[0][0].shape
